...
void MainGame::initSystems()
{
	...
	shaderGeo.init("..\\res\\shaderGeo.vert", "..\\res\\shaderGeo.frag", "..\\res\\shaderGeo.glsl");
	shaderReflect.init("..\\res\\shaderReflection.vert", "..\\res\\shaderReflection.frag");
	shaderExtension.init("..\\res\\shaderExtension.vert", "..\\res\\shaderExtension.frag");
	shaderGeometry.init("..\\res\\shaderGeometry.vert", "..\\res\\shaderGeometry.frag");
	...
	mesh1.loadModel("..\\res\\rock.obj");
	mesh2.loadModel("..\\res\\monkey3.obj");
	mesh3.loadModel("..\\res\\Canoe.obj");
...
void MainGame::processInput()
{
	...
	while (SDL_PollEvent(&evnt)) //get and process events
	{
		...
		case SDL_KEYDOWN:
			...
			{
			case SDLK_w:
				//myCamera.pos += cameraSpeed * myCamera.forward;
			...
			case SDLK_s:
				...
				break;
			case SDLK_a:
				//myCamera.pos -= glm::normalize(glm::cross(myCamera.forward, myCamera.up)) * cameraSpeed;
			...
			case SDLK_d:
				...
				break;
			case SDLK_x:
				myCamera.forward = glm::vec3(myCamera.forward.x + 1, myCamera.forward.y, myCamera.forward.z);
				break;
			case SDLK_z:
				myCamera.forward = glm::vec3(myCamera.forward.x - 1, myCamera.forward.y, myCamera.forward.z);
				break;
...
void MainGame::setGeoShader()
{
	...
	shaderGeo.setFloat("time", counter / 2);
}
...
void MainGame::setExtensionShader()
{
	shaderExtension.setVec3("lightDir", glm::vec3(0.5, 0.5, 0.5));

	shaderExtension.setMat4("model", transform.GetModel());
	shaderExtension.setMat4("view", myCamera.GetView());
	shaderExtension.setMat4("projection", myCamera.GetProjection());
	shaderExtension.setVec3("cameraPos", myCamera.getPos());
}
...
void MainGame::drawGame()
{
	...
	transform.SetPos(glm::vec3(4, -1, 0));
	...
	transform.SetPos(glm::vec3(-4, 0, 0));
	transform.SetRot(glm::vec3(0.0, counter, 0.0));
	...
	transform.SetPos(glm::vec3(-sinf(counter), 3, 0));
	transform.SetRot(glm::vec3(counter, 0.0, 0.0));
